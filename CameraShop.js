...
{
	// For each product with a carousel child set to '1'
	$products.find("product").find("carousel:contains('1')").each(function() {
		// Get the data from the product
		var imgSrc = $(this).parent().find("picture").text();
		var imgTitle = $(this).parent().find("title").text();
		var imgCost = $(this).parent().find("price").text();
		var imgID = $(this).parent().find("id").text();
